...
const {validate} = require("./cardService")
const app = express();
  ...
  return async function (req, res) {
    try {
      ...
      if (!cardNumber) {
        ...
        const validationResult = validate(cardNumber);
        res.json(validationResult.response);
      }
    } catch (error) {
      // Code to handle the error
      console.log("An error occurred: ", error.message);
...
module.exports = { validateCard, validateCardToFile, validateCardUnique };
